...
const wienJobsCrawler: Crawler = {
        ...
        try {
          ...
          const jobs = await page.evaluate(() => {
            const jobElements = document.querySelectorAll(
              '#talentlinkJobsList table.table tbody tr'
            )
            const extractedJobs: WienJobRawJob[] = []
            ...
            jobElements.forEach((jobElement, index) => {
              try {
                ...
                const titleElement = jobElement.querySelector(
                  'th a[data-lumesse-jl-action="detail"]'
                )
  ...
  normalize(raw: ExtractResult): NormalizedItem | null {
    const job = raw as WienJobRawJob
    ...
    const description = [
      job.department && `Dienststelle: ${job.department}`,
      job.deadline && `Bewerbungsfrist: ${job.deadline}`,
      job.publicationDate && `Publikationsdatum: ${job.publicationDate}`
    ]
      .filter(Boolean)
      .join(' | ')

    const fullUrl = job.url.startsWith('http') ? job.url : `https://jobs.wien.gv.at/${job.url}`
...
export default wienJobsCrawler
